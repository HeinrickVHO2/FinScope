...
type AiInterpretationResult =
  ...
  app.post("/api/ai/chat", requireAuth, requireActiveBilling, async (req: any, res) => {
    try {
      ...
      const { clean, isDangerous } = sanitizeUserInput(validation.data.content.trim());
  ...
  // [CÓDIGO ANTIGO REMOVIDO - 640 linhas da arquitetura anterior]
  // Handler GET mantido abaixo
